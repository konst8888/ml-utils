...
def possible_square_shapes(t_dim, img_x, img_y, pd1, k1, s1, pd2, k2, s2):
    min_ = min(img_x, img_y)
    max_ = max(img_x, img_y)
    sizes = []
    for size in range(max(min_ - 100, 0), max_ + 100):
        #print(size)
        conv1_outshape = conv3D_output_size((t_dim, size, size), pd1, k1, s1)
        conv1_outshape = tuple(x / 2 for x in conv1_outshape)
        conv2_outshape = conv3D_output_size(conv1_outshape, pd2, k2, s2)
        conv2_outshape = tuple(x / 2 for x in conv2_outshape)
        if np.all([divmod(x, 1)[1] == 0 for x in conv1_outshape]) and \
        np.all([divmod(x, 1)[1] == 0 for x in conv2_outshape]):
            sizes.append(size)
...
class CNN3d(nn.Module):
    def __init__(self, t_dim=120, img_x=90, img_y=120, drop_p=0.2):
        ...
        f"""self.conv1_outshape={self.conv1_outshape}, self.conv2_outshape={self.conv2_outshape}, possible variants with such time_depth: {possible_square_shapes(
        self.t_dim, self.img_x, self.img_y, 
        self.pd1, self.k1, self.s1, 
        self.pd2, self.k2, self.s2
        )}"""
        ...
        self.conv4 = nn.Conv3d(in_channels=self.ch3, out_channels=self.ch3, kernel_size=self.k2, stride=1,
                               padding=1)
        self.bn4 = nn.BatchNorm3d(self.ch3)
...
class MultiResolution(nn.Module):
    ...
    def __init__(self, drop_p=0.2, fc_hidden1=256*4, fc_hidden2=128*4, num_classes=2, **kwargs):
        ...
        self.fc1 = nn.Linear(self.num_nets * self.fovea.ch3 * np.prod(self.fovea.conv2_outshape).astype(int), fc_hidden1)
        self.fc2 = nn.Linear(self.fc_hidden1, self.fc_hidden2)
...
class VideoNet(nn.Module):
    def __init__(self, t_dim=120, img_x=90, img_y=120, drop_p=0.2, fc_hidden1=256*4, fc_hidden2=128*4, num_classes=2):
