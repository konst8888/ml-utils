        ...
        def compute_loss_and_grads(model, sample, rl):
            with tf.GradientTape() as tape:
                ...
                for i in range(sample.shape[0]):
                    ...
                    img = tf.where(elems == 0, img, elems)
...
#        interpolation='bilinear',
    ...
    model = ReCoNetMobile(a=cfg.model.a, b=cfg.model.b, frn=cfg.model.frn, use_skip=cfg.model.use_skip)
    ...
    print(cfg.dataset.style_path)
    elems = Image.open('elements/haloween/Screenshot_1_part.jpg')
    elems = np.array(elems.resize(IMG_SIZE))
    elems = np.expand_dims(normalize(elems), axis=0)
    elems = elems * 2 - 1
