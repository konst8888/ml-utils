...
def train(
    ...
    data_len = 10
    sample_counter = 0
    print_idx = classes.index('comedy')
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if phase == 'valid' and checkpoint_path is not None:
            torch.save(
                model.state_dict(),
                os.path.join(checkpoint_path, 'tag_txt_epoch_{}_acc_{:.4f}_vs_{:.4f}_loss_{:.4f}_vs_{:.4f}.pth'.format(
                    epoch,
                    train_acc,
                    running_acc * scale_value,
                    train_loss,
...
if __name__ == '__main__':
    ...
    if use_wandb:
        ...
        wandb.init(config=vars(args), project="nlp-tagger") # resume=True
        wandb.save('*.py')

    comp_train = T.Compose([
        prob_wrap(lambda x: re.sub("#[A-Za-zА-Яа-я0-9_]+|@[A-Za-zА-Яа-я0-9_]+","", x), p=1.),
        prob_wrap(drop_half_sentences, p=0.5),
        prob_wrap(augment_context(), p=0.25),
        prob_wrap(augment_w2v(), p=0.25),
        prob_wrap(lambda x: ' '.join(x.split()), p=0.5),
        #prob_wrap(lowercase, p=0.5),
        #prob_wrap(uppercase, p=0.3),
    ])
    comp_test = T.Compose([
        prob_wrap(lambda x: re.sub("#[A-Za-zА-Яа-я0-9_]+|@[A-Za-zА-Яа-я0-9_]+","", x), p=1.),
    ])
    ...
    transforms = (
        lambda x: re.sub("#[A-Za-zА-Яа-я0-9_]+|@[A-Za-zА-Яа-я0-9_]+","", x)
        if random.random() < 1.0
        else x
    )
    dataset_train = TagCharDataset(
        data_path, 
        'train', 
        transforms=comp_train, 
        test_size=test_size
    )
    dataset_test = TagCharDataset(
        data_path, 
        'test', 
        transforms=comp_test, 
        test_size=test_size,
        add_data = {
    ...
    """
    scheduler = ReduceLROnPlateau(optimizer, 'min', patience=args.patience, factor=args.factor, min_lr=5e-5)
    
    class_proportion = dataset_train.class_proportion
    class_proportion[classes.index('comedy')] /= 5
    class_proportion[classes.index('video_games')] /= 2
