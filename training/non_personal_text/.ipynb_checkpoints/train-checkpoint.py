...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if phase == 'valid' and checkpoint_path is not None:
            torch.save(
                model.state_dict(),
                os.path.join(checkpoint_path, 'np_txt_epoch_{}_acc_{:.4f}_vs_{:.4f}_loss_{:.4f}_vs_{:.4f}.pth'.format(
                    epoch,
                    train_acc,
                    running_acc * scale_value,
                    train_loss,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--force-vocab', help='Whether to retrain vocab', action='store_true')
    ...
    if use_wandb:
        ...
        wandb.init(config=vars(args), project="nlp-non-personal") # resume=True
        wandb.save('*.py')
    ...
    model = AttnBiLSTM(
        embedding_size=300, 
        hidden_size=100, 
        total_words=dataset_train.bpe.vocab_size(), 
