	...
	def __getitem__(self, idx):
		...
		for i in range(0, len(self.numlist)):
			...
			if(idx < (self.numlist[i]-1)):
				...
				img1 = torch.from_numpy(transform.resize(img1, self.shape, order=3)).to(device).permute(2, 0, 1).float()
				img2 = torch.from_numpy(transform.resize(img2, self.shape, order=3)).to(device).permute(2, 0, 1).float()
				mask = torch.from_numpy(transform.resize(mask, self.shape, order=3)).to(device).float()
				flow = read(flowpath+"frame_"+num1+".flo")
				# bilinear interpolation is default
				originalflow=torch.from_numpy(flow)                
				flow = torch.from_numpy(transform.resize(flow, self.shape, order=3)).to(device).permute(2,0,1).float()
